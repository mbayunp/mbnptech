...
const accentMap: Record<string, { icon: string; tag: string }> = {
  blue: { icon: 'bg-blue-50 text-blue-600', tag: 'bg-blue-50 text-blue-700 border-blue-100' },
  indigo: { icon: 'bg-indigo-50 text-indigo-600', tag: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
  violet: { icon: 'bg-violet-50 text-violet-600', tag: 'bg-violet-50 text-violet-700 border-violet-100' },
  emerald: { icon: 'bg-emerald-50 text-emerald-600', tag: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
  amber: { icon: 'bg-amber-50 text-amber-600', tag: 'bg-amber-50 text-amber-700 border-amber-100' },
};

const CoreServices = () => (
  <section className="py-20 md:py-28 bg-white relative z-20 -mt-8 rounded-t-[2.5rem] md:rounded-t-[3.5rem] border-t border-slate-100">
    <div className="absolute top-[-5%] right-[5%] w-[400px] h-[400px] bg-gradient-to-br from-indigo-100/40 to-violet-100/20 rounded-full blur-[120px] pointer-events-none" />
    <div className="container mx-auto px-5 md:px-8 max-w-6xl relative z-10">
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-100px' }} className="text-center mb-14 md:mb-16">
        <motion.p variants={fadeUp} className="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Core Services</motion.p>
        <motion.h2 variants={fadeUp} className="font-display text-3xl md:text-5xl font-black text-slate-900 tracking-tight">Keahlian & Spesialisasi</motion.h2>
      </motion.div>
          ...
          return (
            <motion.div key={idx} variants={fadeUp}
              ...
              <div className="mb-5 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                <p className="text-[10px] text-slate-400 uppercase tracking-widest mb-2.5 font-black">Fitur Kunci</p>
                <ul className="space-y-1.5">
                  {srv.features.map((f, i) => (
                    <li key={i} className="text-xs text-slate-700 font-semibold flex items-center gap-2">
                      <FiCheck className="text-emerald-500 shrink-0" />{f}
                    </li>
                  ))}
                </ul>
              </div>
              <span className={`inline-block px-3 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-xl border ${colors.tag}`}>{srv.examples}</span>
            </motion.div>
          );
        })}
      </motion.div>
...
const IndustrySolutions = () => (
  <section className="py-20 md:py-28 bg-slate-50 border-t border-slate-100 relative overflow-hidden">
    <div className="absolute bottom-0 left-1/4 w-[500px] h-[400px] bg-gradient-to-tr from-sky-100/40 to-indigo-100/20 rounded-full blur-[120px] pointer-events-none" />
    <div className="container mx-auto px-5 md:px-8 max-w-6xl relative z-10">
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-100px' }} className="mb-12 md:mb-16 md:w-2/3">
        <motion.p variants={fadeUp} className="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Industry Solutions</motion.p>
        <motion.h2 variants={fadeUp} className="font-display text-3xl md:text-5xl font-black text-slate-900 tracking-tight">Menyelesaikan Masalah di Berbagai Sektor</motion.h2>
      </motion.div>
        ...
        {industries.map((ind, idx) => (
          <motion.div key={idx} variants={fadeUp}
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm leading-relaxed font-medium mb-4">{ind.desc}</p>
              <span className="inline-block px-3 py-1.5 bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-wider rounded-lg border border-blue-100">
...
const DevelopmentProcess = () => (
  <section className="py-20 md:py-28 bg-[#0F172A] text-white overflow-hidden relative">
...
const ServicesFAQ = () => {
  ...
  return (
    <section className="py-20 md:py-28 bg-slate-50 border-t border-slate-100 relative overflow-hidden">
      <div className="absolute top-0 right-1/4 w-[400px] h-[300px] bg-blue-50/80 rounded-full blur-[120px] pointer-events-none" />
...
const ServicesCTA = () => (
  <section className="py-24 md:py-36 bg-white relative overflow-hidden border-t border-slate-100">
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] md:w-[900px] md:h-[900px] bg-gradient-to-tr from-sky-100/60 to-blue-100/40 rounded-full blur-3xl -z-10 pointer-events-none" />
