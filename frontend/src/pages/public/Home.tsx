...
// === HERO ===
const HeroSection = () => (
  <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden bg-[#fafafa]">
    {/* Mesh Gradient Blobs */}
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      <div className="absolute -top-[20%] -left-[10%] w-[70%] h-[70%] bg-gradient-to-br from-blue-400/25 to-indigo-400/10 rounded-full blur-[120px]" />
      <div className="absolute top-[30%] -right-[10%] w-[60%] h-[60%] bg-gradient-to-bl from-violet-400/20 to-sky-300/10 rounded-full blur-[100px]" />
      <div className="absolute -bottom-[10%] left-[20%] w-[55%] h-[50%] bg-gradient-to-tr from-emerald-300/15 to-cyan-400/10 rounded-full blur-[120px]" />
    </div>
    ...
    <div className="container mx-auto px-5 md:px-8 relative z-10 pt-8">
      ...
      >
        {/* Live Badge */}
        <motion.div variants={fadeUp} className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full bg-white/70 backdrop-blur-md border border-slate-200/60 shadow-sm mb-10 hover:bg-white hover:scale-105 transition-all cursor-default">
          <span className="relative flex h-2.5 w-2.5">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-500 opacity-75" />
            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-600" />
          </span>
          <span className="text-xs font-black text-slate-700 tracking-widest uppercase">MBNP Tech • IT Solutions</span>
        </motion.div>
        ...
        <motion.p variants={fadeUp} className="text-base sm:text-lg md:text-xl text-slate-500 mb-12 max-w-2xl mx-auto leading-relaxed font-medium">
        ...
        <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <Link to="/contact" className="group relative w-full sm:w-auto px-8 py-4 bg-slate-900 text-white rounded-2xl font-bold text-base overflow-hidden transition-all hover:scale-105 hover:shadow-2xl hover:shadow-slate-900/25 flex justify-center items-center gap-2.5">
            <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            <span className="relative z-10">Mulai Kolaborasi</span>
            <ArrowRight size={16} className="relative z-10 group-hover:translate-x-1 transition-transform" />
          </Link>
          <Link to="/projects" className="w-full sm:w-auto px-8 py-4 bg-white/60 backdrop-blur-sm text-slate-800 border border-slate-200/80 rounded-2xl font-bold text-base hover:bg-white hover:border-slate-300 transition-all hover:scale-105 shadow-sm flex justify-center items-center gap-2">
...
const bentoItems = [
  {
    ...
    bg: 'bg-gradient-to-br from-slate-900 to-[#0B1120]', iconBg: 'bg-white/10 text-white', glow: 'bg-blue-500/20',
  },
  {
    icon: <BarChart2 size={20} />, label: 'Portal Data', wide: false, dark: false,
    desc: 'Dashboard & open data portal untuk Pemerintah.',
    bg: 'bg-gradient-to-br from-blue-50 to-indigo-50/50', iconBg: 'bg-blue-600 text-white', glow: '',
  },
  {
    icon: <Zap size={20} />, label: 'Modern UI/UX', wide: false, dark: false,
    desc: 'Eksplorasi antarmuka estetik yang interaktif.',
    bg: 'bg-gradient-to-br from-emerald-50 to-teal-50/50', iconBg: 'bg-emerald-500 text-white', glow: '',
  },
  {
    ...
    bg: 'bg-white', iconBg: 'bg-slate-100 text-slate-700', glow: '',
  },
];

const FeaturesBentoSection = () => (
  <section className="py-20 md:py-28 bg-white relative z-20 -mt-8 md:-mt-12 rounded-t-[2.5rem] md:rounded-t-[3.5rem] border-t border-slate-100 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.05)]">
    <div className="container mx-auto px-5 md:px-8 max-w-6xl">
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-100px' }} className="text-center mb-14 md:mb-16">
        <motion.p variants={fadeUp} className="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Layanan Unggulan</motion.p>
        <motion.h2 variants={fadeUp} className="font-display text-3xl md:text-5xl font-black text-slate-900 tracking-tight">Expertise Teknologi Modern</motion.h2>
        <motion.p variants={fadeUp} className="text-base md:text-lg text-slate-500 max-w-2xl mx-auto mt-4">Solusi komprehensif dari perancangan arsitektur hingga deployment produksi.</motion.p>
      </motion.div>
        ...
        {bentoItems.map((item, i) => (
          ...
          >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between min-h-[160px]">
              ...
              <div>
                <h3 className={`font-display text-xl md:text-2xl font-black mb-2 ${item.dark ? 'text-white' : 'text-slate-900'}`}>{item.label}</h3>
                <p className={`text-sm md:text-base leading-relaxed max-w-sm ${item.dark ? 'text-slate-400' : 'text-slate-600'}`}>{item.desc}</p>
              </div>
            </div>
          </motion.div>
        ))}
...
const FeaturedProjectSection = () => (
  <section className="py-20 md:py-28 bg-slate-50 border-y border-slate-100 relative overflow-hidden">
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
      <div className="absolute top-[-15%] left-[40%] w-[50%] h-[50%] bg-indigo-400/10 rounded-full blur-[120px]" />
      <div className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[50%] bg-violet-400/10 rounded-full blur-[100px]" />
    </div>
    <div className="container mx-auto px-5 md:px-8 max-w-7xl relative z-10">
      <div className="mb-12 md:mb-16 md:flex md:items-end md:justify-between text-center md:text-left">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-100px' }} className="max-w-2xl mx-auto md:mx-0">
          <motion.p variants={fadeUp} className="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Showcase Utama</motion.p>
          <motion.h2 variants={fadeUp} className="font-display text-3xl md:text-5xl font-black text-slate-900 tracking-tight">Karya Terbaik Tersorot</motion.h2>
        </motion.div>
        <div className="mt-6 md:mt-0 hidden md:block">
          <Link to="/projects" className="inline-flex items-center gap-2 px-5 py-3 bg-white text-slate-800 font-bold border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm text-sm">
      ...
      <motion.div
        ...
        className="bg-[#0F172A] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-14 flex flex-col lg:flex-row gap-8 md:gap-12 items-center relative overflow-hidden shadow-2xl shadow-blue-900/10 border border-slate-800"
      >
        ...
        <div className="w-full lg:w-[55%] relative z-10">
          <motion.div whileHover={{ y: -6, scale: 1.02 }} transition={{ type: 'spring', stiffness: 200, damping: 20 }} className="rounded-2xl bg-slate-800/80 border border-slate-700/80 shadow-2xl overflow-hidden">
            <div className="h-9 bg-slate-900 border-b border-slate-700/80 flex items-center px-4 gap-1.5">
              <div className="w-3 h-3 rounded-full bg-[#FF5F56]" />
              <div className="w-3 h-3 rounded-full bg-[#FFBD2E]" />
              <div className="w-3 h-3 rounded-full bg-[#27C93F]" />
              <div className="ml-4 flex-1 flex justify-center">
                <div className="w-1/2 h-5 bg-slate-800 rounded-md border border-slate-700 text-[9px] text-slate-500 flex items-center justify-center tracking-widest font-mono">picmestudio.id</div>
              </div>
            </div>
  ...
  <section className="py-20 bg-white">
    <div className="container mx-auto px-5 md:px-8 max-w-6xl">
      <motion.h2 variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }}
        className="font-display text-2xl md:text-3xl font-black text-slate-900 tracking-tight mb-10 text-center">
        Lebih Banyak Projek
      </motion.h2>
...
const FlowSection = () => (
  <section className="py-20 md:py-28 bg-[#0F172A] text-white overflow-hidden relative border-t border-slate-800">
      ...
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }}
        ...
        {flowSteps.map((s, idx) => (
          <motion.div key={idx} variants={fadeUp}
            ...
            className="relative group text-center md:text-left border-b md:border-none border-slate-800 pb-6 md:pb-0 last:border-none">
            ...
            <div className="text-4xl md:text-5xl font-black text-slate-800/70 mb-2 group-hover:text-blue-500/20 transition-colors">{s.n}</div>
            ...
            {idx < 3 && <div className="hidden md:block absolute top-12 left-16 right-4 h-[1px] bg-gradient-to-r from-slate-700 to-transparent" />}
          </motion.div>
        ))}
      </motion.div>
    </div>
...
const ModernCTAButton = () => (
  <section className="py-24 md:py-36 bg-white relative overflow-hidden">
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] md:w-[900px] md:h-[900px] bg-gradient-to-tr from-blue-100/60 to-violet-100/40 rounded-full blur-3xl -z-10" />
    <div className="container mx-auto px-5 md:px-8 max-w-4xl text-center relative z-10">
      <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-80px' }}>
        <motion.h2 variants={fadeUp} className="font-display text-4xl sm:text-5xl md:text-7xl font-black text-slate-900 mb-6 md:mb-8 tracking-tighter">
        ...
        <motion.p variants={fadeUp} className="text-base sm:text-lg md:text-xl text-slate-500 mb-10 md:mb-14 max-w-2xl mx-auto font-medium">
